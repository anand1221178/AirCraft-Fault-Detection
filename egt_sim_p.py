...
def simulate_engine_data(duration_minutes, frequency_hz, scenario='Normal', params=PARAMS, dropout_rate=DROPOUT_RATE):
    ...
    for sensor in ['EGT', 'N2', 'OilPressure', 'Vibration']: # Base names
         # Reported vibration needs Vib1 and Vib2
        if sensor == 'Vibration':
    ...
    for i in range(num_data_points):
        ...
        for sensor, p in params.items():
            ...
            if sensor_failing == sensor and p['has_hmm']:
                ...
                if sensor_fail_progress < 0.1: sensor_health = 'OK'
                elif sensor_fail_progress < 0.9: sensor_health = 'Degraded'
                else: sensor_health = 'Failed'

                if sensor == 'EGT': data['EGT_Sensor_Health'][i] = sensor_health
                if sensor == 'Vibration': data['Vibration_Sensor_Health'][i] = sensor_health
            ...
            if sensor == 'Vibration':
                 ...
                 if np.random.rand() < dropout_rate: vib1_final = np.nan
                 if np.random.rand() < dropout_rate: vib2_final = np.nan
                ...
                # Apply dropout
                if np.random.rand() < dropout_rate: reported_val_final = np.nan
