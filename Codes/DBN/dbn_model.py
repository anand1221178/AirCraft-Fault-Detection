...
def define_dbn_structure():
    ...
    dbn_structure = [
        ...
        (('EGT_Sensor_Health', 0), ('EGT_Sensor_Health', 1)),
        (('Vibration_Sensor_Health', 0), ('Vibration_Sensor_Health', 1)),
...
def define_initial_cpts():
    """Defines simplified CPTs (same as previous corrected version)"""
    cpt_list = []
    # P(Engine_Core_Health(0))
    ...
    # P(EGT_C_Discrete | Engine_Core_Health)
    cpd_egt_obs = TabularCPD(variable=('EGT_C_Discrete', 0), variable_card=3, values=[[0.10, 0.10, 0.05], [0.80, 0.50, 0.25], [0.10, 0.40, 0.70]], evidence=[('Engine_Core_Health', 0)], evidence_card=[3])
    cpt_list.append(cpd_egt_obs)
    # P(N2_PctRPM_Discrete | Engine_Core_Health)
    cpd_n2_obs = TabularCPD(variable=('N2_PctRPM_Discrete', 0), variable_card=3, values=[[0.10, 0.10, 0.15], [0.80, 0.80, 0.75], [0.10, 0.10, 0.10]], evidence=[('Engine_Core_Health', 0)], evidence_card=[3])
    cpt_list.append(cpd_n2_obs)
    # P(OilPressure_PSI_Discrete | Lubrication_System_Health)
    cpd_oilp_obs = TabularCPD(variable=('OilPressure_PSI_Discrete', 0), variable_card=3, values=[[0.10, 0.80], [0.80, 0.15], [0.10, 0.05]], evidence=[('Lubrication_System_Health', 0)], evidence_card=[2])
    cpt_list.append(cpd_oilp_obs)
    # P(Vib1_IPS_Discrete | Engine_Core_Health)
    cpd_vib1_obs = TabularCPD(variable=('Vib1_IPS_Discrete', 0), variable_card=3, values=[[0.60, 0.10, 0.05], [0.35, 0.60, 0.25], [0.05, 0.30, 0.70]], evidence=[('Engine_Core_Health', 0)], evidence_card=[3])
    cpt_list.append(cpd_vib1_obs)
    # P(Vib2_IPS_Discrete | Engine_Core_Health)
    cpd_vib2_obs = TabularCPD(variable=('Vib2_IPS_Discrete', 0), variable_card=3, values=[[0.60, 0.10, 0.05], [0.35, 0.60, 0.25], [0.05, 0.30, 0.70]], evidence=[('Engine_Core_Health', 0)], evidence_card=[3])
    ...
    # P(EGT_Sensor_Health(t) | EGT_Sensor_Health(t-1))
    ...
    # P(Vibration_Sensor_Health(t) | Vibration_Sensor_Health(t-1))
    ...
    # P(EGT_C_Discrete(1) | Engine_Core_Health(1))
    cpd_egt_obs_t1 = TabularCPD(variable=('EGT_C_Discrete', 1), variable_card=3, values=cpd_egt_obs.values, evidence=[('Engine_Core_Health', 1)], evidence_card=[3])
    cpt_list.append(cpd_egt_obs_t1)
    # P(N2_PctRPM_Discrete(1) | Engine_Core_Health(1))
    cpd_n2_obs_t1 = TabularCPD(variable=('N2_PctRPM_Discrete', 1), variable_card=3, values=cpd_n2_obs.values, evidence=[('Engine_Core_Health', 1)], evidence_card=[3])
    cpt_list.append(cpd_n2_obs_t1)
    # P(OilPressure_PSI_Discrete(1) | Lubrication_System_Health(1))
    cpd_oilp_obs_t1 = TabularCPD(variable=('OilPressure_PSI_Discrete', 1), variable_card=3, values=cpd_oilp_obs.values, evidence=[('Lubrication_System_Health', 1)], evidence_card=[2])
    cpt_list.append(cpd_oilp_obs_t1)
    # P(Vib1_IPS_Discrete(1) | Engine_Core_Health(1))
    cpd_vib1_obs_t1 = TabularCPD(variable=('Vib1_IPS_Discrete', 1), variable_card=3, values=cpd_vib1_obs.values, evidence=[('Engine_Core_Health', 1)], evidence_card=[3])
    cpt_list.append(cpd_vib1_obs_t1)
    # P(Vib2_IPS_Discrete(1) | Engine_Core_Health(1))
    cpd_vib2_obs_t1 = TabularCPD(variable=('Vib2_IPS_Discrete', 1), variable_card=3, values=cpd_vib2_obs.values, evidence=[('Engine_Core_Health', 1)], evidence_card=[3])
